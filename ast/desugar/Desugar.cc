...
ExpressionPtr symbol2Proc(DesugarContext dctx, ExpressionPtr expr) {
    ...
    ExpressionPtr body = MK::Send0(loc, std::move(recv), name);
    ...
    try {
        ...
        typecase(
            ...
            [&](parser::OrAsgn *orAsgn) {
                ...
                if (auto s = cast_tree<Send>(recv)) {
                    ...
                    if (ident && ident->kind == UnresolvedIdent::Kind::Class) {
                        ...
                        cond = std::move(res);
                        // ..and because the send above "hides" the type of the variable
                        // from type inference, we need to help out the inferencer here.
                        elsep = MK::Send1(loc, MK::T(loc), core::Names::must(), MK::cpRef(recv));
                    } else {
                        cond = MK::cpRef(recv);
                        elsep = MK::cpRef(recv);
                    }
            ...
            [&](parser::DSymbol *dsymbol) {
                ...
                ExpressionPtr res = MK::Send0(loc, std::move(str), core::Names::intern());
            ...
            [&](parser::Yield *ret) {
                ...
                ExpressionPtr res = MK::Send(loc, std::move(recv), core::Names::call(), args.size(), std::move(args));
                result = std::move(res);
            },
